...
public class OrdinalChartReport extends SingleCriterionReport<AmSerialChart> {
    ...
    public OrdinalChartReport(Criterion criterion, List<UserChoices> userChoiceses) {
        super(criterion, userChoiceses);
    }

    @Override
    public void run() {
        if(!(criterion instanceof OrdinalScaledCriterion))
            return;

        this.criterion = (OrdinalScaledCriterion)criterion;
